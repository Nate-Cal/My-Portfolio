...
existing_dnis = set(dni.lower() for dni in worksheet.col_values(dni_col)[2:])
...
# Data containers
dates, times, names, emails, phones, ages, province_list, dni_list, cities = [], [], [], [], [], [], [], [], []
...
with open(csv_path, newline='', encoding=detected_encoding) as csvfile:
    ...
    for row in reader:
        ...
        if dob_str:
            ...
            ages.append(dob_str)
        province_list.append(province_value)
        dni_list.append(dni_value)
        cities.append(row.get("ciudad", "").strip())
...
max_rows = max(len(names), len(ages), len(dni_list), len(emails), len(phones), len(province_list), len(cities))
...
# Should I add a temp city column? Might break existing formulas!
#update_column()
